    ...
    <Layout>
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-extrabold text-foreground mb-1 tracking-tight">Water Bodies</h1>
            <p className="text-muted-foreground">Browse urban water monitoring records across Maharashtra — data from AI Kosh</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <Select value={filterDistrict} onValueChange={setFilterDistrict}>
            ...
            <SelectContent>
              ...
              {MAHARASHTRA_DISTRICTS.map((d) => <SelectItem key={d} value={d}>{d}</SelectItem>)}
            </SelectContent>
          </Select>
        ...
        <div>
          <p className="text-sm text-muted-foreground mb-4">Showing {filteredWaterBodies.length} of {waterBodies.length} water bodies</p>
          {filteredWaterBodies.length === 0 ? (
            ...
            <div className="bg-card p-6 rounded-xl shadow-card">
              <WaterBodyMap waterBodies={filteredWaterBodies} height="600px" onMarkerClick={(id) => navigate(`/water-body/${id}`)} />
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredWaterBodies.map((wb) => <WaterBodyCard key={wb.id} waterBody={wb} />)}
            </div>
          )}
        </div>
      </div>
